 ...
 *  - Grade cheia: 15 em 15 minutos (00, 15, 30, 45).
 *  - Sempre oferece apenas 2 opções por vez.
 *  - Telefone só aparece quando o paciente pede humano ou após 2 confusões.
 */
...
setInterval(() => {
  ...
  for (const [k, v] of SESSIONS.entries()) {
    if (now - v.updatedAt > 1000 * 60 * 60 * 6) SESSIONS.delete(k);
  }
}, 1000 * 60 * 30);
...
const ReasonSchema = z.enum([
  'Consulta', 'Vasectomia – avaliação', 'Litíase/Rim – avaliação',
  'HPB/Próstata – avaliação', 'Disfunção Erétil – avaliação', 'Pediátrica – avaliação',
]);
...
function wantsHuman(text) {
  const t = (text || '').toLowerCase();
  return /(humano|atendente|pessoa|secretaria|recep[cç][aã]o|telefone|ligar|whatsapp|falar com|suporte)/i.test(t);
}
function incConfused(session, phone) {
  session.data.confusedCount = (session.data.confusedCount || 0) + 1;
  setSession(phone, session);
  return session.data.confusedCount;
}
function humanHandoffMessage() {
  return CLINIC_PHONE
    ? `Sem problemas! Você pode falar com nossa equipe humana pelo ${CLINIC_PHONE}.`
    : 'Sem problemas! Posso pedir para nossa equipe humana entrar em contato.';
}
...
app.post('/whatsapp', async (req, res) => {
  ...
  // Se o usuário pedir humano explicitamente
  if (wantsHuman(body)) {
    res.set('Content-Type','text/xml').send(twimlMessage(humanHandoffMessage()));
    return;
  }
  ...
  try {
    ...
    if (session.state === 'ask_name') {
      ...
      if (name.length < 2) { res.set('Content-Type','text/xml').send(twimlMessage(await humanize('Pode me dizer seu nome completo?'))); return; }
      session.data.name = name;
      if (ACCEPT_HEALTH_PLANS) {
        session.state = 'ask_insurance'; setSession(phone, session);
        res.set('Content-Type','text/xml').send(twimlMessage(await humanize(`Obrigada, ${name.split(' ')[0]}! O atendimento será por convênio ou particular?`))); return;
      }
      session.state = 'ask_reason'; setSession(phone, session);
      res.set('Content-Type','text/xml').send(twimlMessage(await humanize('Ótimo! Qual o motivo da consulta? Se já tiver um dia em mente, pode dizer “amanhã”, “próxima quarta”, etc.'))); return;
    }
    ...
    if (session.state === 'ask_insurance') {
      ...
      // Se já informar o nome do plano aqui (ex.: ipe, ipergs, unimed...), pula direto
      const knownPlans = /(ipe|ipergs|unimed|bradesco|amil|cassi|sulamerica|hapvida|prevent)/i;
      if (knownPlans.test(norm)) {
        session.data.billing = { mode: 'convenio' };
        session.data.planName = raw; // guarda como a pessoa escreveu
        session.state = 'ask_reason'; setSession(phone, session);
        res.set('Content-Type','text/xml').send(twimlMessage(await humanize('Perfeito. Qual o motivo da consulta? Se quiser, já diga um dia (ex.: "próxima quarta").'))); return;
      }
      ...
      if (t.includes('convenio') || t.includes('convênio') || t.includes('plano')) {
        session.data.billing = { mode:'convenio' };
        session.state='ask_plan_name'; setSession(phone, session);
        res.set('Content-Type','text/xml').send(twimlMessage(await humanize('Qual é o nome do seu plano?'))); return;
      }
    ...
    if (session.state === 'ask_plan_name') {
      session.data.planName = body.trim();
      session.state='ask_reason'; setSession(phone, session);
      res.set('Content-Type','text/xml').send(twimlMessage(await humanize('Obrigado! Qual o motivo da consulta? Se já tiver um dia em mente, pode me dizer.'))); return;
    }

    // ===== ask_reason -> propose_slots =====
    if (session.state === 'ask_reason') {
      const reason = body.trim();
      const parsed = ReasonSchema.safeParse(reason);
      session.data.reason = parsed.success ? parsed.data : reason;
      session.state = 'propose_slots'; setSession(phone, session);
      ...
      if (suggestions.length === 0) {
        res.set('Content-Type','text/xml').send(twimlMessage(await humanize('Não encontrei horários livres agora. Pode me dizer um dia que prefira, tipo “amanhã” ou “próxima quarta”?'))); return;
      }
    ...
    if (session.state === 'propose_slots') {
      ...
      if (preferred) {
        const calendarId = getCalendarId();
        let base = preferred; let info = null;
        const minIpe = DateTime.now().setZone(TZ).plus({ weeks: 2 }).startOf('day').set({ hour: 9, minute: 0 });
        if (isIpePlan && preferred < minIpe) { base = minIpe; info = `Para ${session.data.planName}, começamos a partir de ${minIpe.setLocale('pt-BR').toFormat('dd/LL')}.`; }
      ...
      session.data.chosen = chosen; session.state = 'confirm_slot'; setSession(phone, session);
    ...
    if (session.state === 'confirm_slot') {
      ...
      if (yes) {
        ...
        const descriptionParts = [ `Origem: WhatsApp`, `Telefone: ${phone}`, `Paciente: ${session.data.name || ''}` ];
        ...
        if (!stillFree) {
          session.state = 'ask_reason'; setSession(phone, session);
          res.set('Content-Type','text/xml').send(twimlMessage(await humanize('Esse horário acabou de ficar indisponível. Me diga o motivo da consulta e eu te ofereço novas opções, tudo bem?')));
          return;
        }
    ...
    if (session.state === 'booked') {
      res.set('Content-Type','text/xml').send(twimlMessage(await humanize('Você já tem um agendamento confirmado. Para iniciar outro ou remarcar, envie “menu” ou escreva “remarcar”.')));
      return;
    }
